const Store=(()=>{const key="vibrinova-data";const def={theme:{mode:"dark",primary:"#6EE7F9",accent:"#8B5CF6"},onboarded:false,lastPage:"index",activeProfileId:"",profiles:[{id:"p1",name:"KIDS",kids:true,avatar:"K",password:null},{id:"p2",name:"Profile 1",kids:false,avatar:"P",password:null},{id:"p3",name:"Profile 2",kids:false,avatar:"P",password:null},{id:"p4",name:"Profile 3",kids:false,avatar:"P",password:null}],devices:[],byProfile:{},chat:{messages:[],recommendations:[]}};const load=()=>{try{const s=JSON.parse(localStorage.getItem(key))||def;if(typeof s.theme==="string")s.theme={mode:s.theme,primary:def.theme.primary,accent:def.theme.accent};if(!s.byProfile)s.byProfile={};(s.profiles||def.profiles).forEach(p=>{if(!s.byProfile[p.id])s.byProfile[p.id]={histories:{search:{},watch:[]}}});return s}catch{return def}};let state=load();const save=()=>localStorage.setItem(key,JSON.stringify(state));return{get:()=>state,set:fn=>{state=fn({...state});save();return state}}})();

const U={uid:()=>Math.random().toString(36).slice(2,10),code8:()=>{const c="ABCDEFGHJKLMNPQRSTUVWXYZ23456789";let o="";for(let i=0;i<8;i++)o+=c[Math.floor(Math.random()*c.length)];return o},tags:t=>{const r=/@([A-Za-z0-9][A-Za-z0-9 ':.-]{0,49})/g,s=new Set();let m;while((m=r.exec(t))!==null)s.add(m[1].trim());return[...s]},el:(s,r=document)=>r.querySelector(s),els:(s,r=document)=>Array.from(r.querySelectorAll(s)),qs:k=>new URLSearchParams(location.search).get(k)};

/* Theme */
function applyTheme(){const t=Store.get().theme;document.documentElement.dataset.theme=t.mode;document.documentElement.style.setProperty("--primary",t.primary);document.documentElement.style.setProperty("--accent",t.accent)}
...
function renderChat(){const w=U.el("#chat-messages");if(!w)return;const {profiles,activeProfileId,chat}=Store.get();const me=profiles.find(p=>p.id===activeProfileId);w.innerHTML=`<div class="bubble other">Welcome to VibriNova Chat!</div><div class="bubble other">Use @Title to recommend</div>`;chat.messages.forEach(m=>{const self=(m.sender===me?.name||m.sender===me?.id);const d=document.createElement("div");d.className="bubble "+(self?"self":"other");d.textContent=m.text;w.appendChild(d)});w.scrollTop=w.scrollHeight}
function bindChat(){const inp=U.el("#chat-input"),form=U.el("#chat-form"),w=U.el("#chat-messages");if(!form||!inp)return;form.addEventListener("submit",e=>{e.preventDefault();const v=inp.value.trim();if(!v)return;const me=Store.get().profiles.find(p=>p.id===Store.get().activeProfileId);addMsg(me?.name||"Me",v);inp.value="";renderChat();setTimeout(()=>w.scrollTop=w.scrollHeight,50)});const R=U.el("#recs");if(R){const {chat}=Store.get();R.innerHTML=chat.recommendations.length?chat.recommendations.map(r=>`<div class="row card section" style="align-items:center;"><div><div><strong>${r.title}</strong></div><div class="text-muted" style="font-size:12px">by ${r.from}</div></div><button class="btn">Add</button></div>`).join(""):`<p class="text-muted">No recommendations yet.</p>`}renderChat()}
